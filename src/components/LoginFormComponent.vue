<template>
  <section class="modal">
    <div class="inner">
      <button class="close" @click="setLoginModalVisible">
        <svg
          width="24"
          height="24"
          viewBox="0 0 40 40"
      ...
      <v-form class="form" v-model="valid">
        ...
        <div class="tools">
          <div class="tools_box">
            <input type="checkbox" name="remember" class="checkbox" v-model="remember" />
        ...
        <button class="button" :disabled="!valid" @click.prevent="loginUser">
          {{ t("header.login") }}
        </button>
        <div class="text">
          {{ t("forms.not_register") }}
          <button class="link" @click.prevent="setRegisterModalVisible">
            {{ t("header.register") }}
          </button>
        </div>
      </v-form>
...
const loginUser = async () => {
  ...
  const response = await fetch("https://api.megagamer.org/api/auth/login", {
    method: "POST",
    headers: {
