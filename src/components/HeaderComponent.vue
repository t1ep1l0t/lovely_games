<template>
  <header class="header">
    <div class="container">
      <nav class="nav">
        <div class="buttons">
          <button class="button" v-if="!user" @click="setLoginModalVisible">
            {{ t('header.login') }}
          </button>
          <router-link class="profile" v-else :to="translation.i18nRoute({ name: 'profile' })">
            <img src="../assets/images/no_avatar.png" alt="" class="avatar" v-if="!user.image" />
            <img :src="user.image" alt="" class="avatar" v-else />
          </router-link>
        </div>
        <div class="links">
          <router-link class="link" :to="translation.i18nRoute({ name: 'home' })">
            {{ t('header.home') }}
          </router-link>
          <router-link
            class="link"
        ...
        <div class="buttons">
          <button class="button" v-if="!user" @click="setRegisterModalVisible">
            {{ t('header.register') }}
          </button>
          <router-link class="button" v-else :to="translation.i18nRoute({ name: 'profile' })">
            {{ user.balance }}
            <svg
              width="18"
              height="18"
